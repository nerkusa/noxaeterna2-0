...
var tabs=[{id:"character",l:"Профиль",ic:<IconChar/>},{id:"combat",l:"Бой",ic:<IconBattle/>},{id:"inventory",l:"Инв.",ic:<IconInv/>},{id:"party",l:"Группа",ic:<IconParty/>},{id:"library",l:"Лор",ic:<IconLib/>},{id:"guide",l:"Памятка",ic:<span style={{fontSize:15}}>📋</span>}];
...
{!isGMv&&<Sidebar char={c} save={pr.save} onRoll={sRP} addLog={pr.addLog}/>}
<div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column"}}>
<div style={{display:"flex",alignItems:"center",padding:"6px 8px 2px",borderBottom:"2px solid #34374a",background:"#161826",gap:5}}>{pr.onBack&&<button onClick={pr.onBack} style={{background:"none",border:"none",fontSize:14,cursor:"pointer",color:"#9397ab"}}>←</button>}<div style={{flex:1,textAlign:"center"}}><div style={{fontSize:12,fontFamily:"'Inter',sans-serif",fontWeight:900}}>{c.name||"Герой"}{pr.isGM&&<span style={{fontSize:8,color:"#7c3aed",marginLeft:3}}>ГМ</span>}</div><div style={{fontSize:8,color:"#9397ab"}}>Ур.{c.level}</div></div></div>
<div style={{display:"flex",gap:1,padding:"2px 2px 0",background:"#232532",borderBottom:"1px solid #34374a"}}>{tabs.map(function(t){return <button key={t.id} onClick={function(){sT(t.id)}} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1,padding:"4px 1px",border:"none",borderRadius:"6px 6px 0 0",cursor:"pointer",fontFamily:"'Inter',sans-serif",background:tab===t.id?"#2b2e40":"transparent",color:tab===t.id?"#cbb990":"#9397ab",fontWeight:tab===t.id?700:500,fontSize:8}}>{t.ic}<span>{t.l}</span></button>})}</div>
