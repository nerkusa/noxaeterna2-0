...
const ATYPES = [
  ...
  { id: 'roll_charisma', name: 'Бросок Убеждение +5 (1/день)' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontFamily: "'Inter',sans-serif", fontWeight: 900, fontSize: 16, color: CLR }}>🎭 Классы</div>
      ...
      {profs.map(function (p) {
        ...
        return (
          <div key={p.id} style={{ border: '2px solid #34374a', borderRadius: 9, background: '#1b1d29', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 9px' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 12, color: '#e9e9ed', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
                <div style={{ fontSize: 8, color: '#9397ab' }}>{isNone ? 'базовый' : (p.abN || '—') + (p.ab ? ' · ' + p.ab : '')}</div>
              </div>
              ...
              <div style={{ padding: '0 9px 9px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {p.abilityType === 'custom_roll' && (
                  <div style={{ background: '#1c1530', borderRadius: 6, padding: '6px 8px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    <div style={{ fontSize: 8, color: '#75798c' }}>Пример: «1d8 + INT + Lore». Бросает игрок со своими характеристиками.</div>
                  </div>
                )}
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {SD.map(function (s) {
                      ...
                      return <button key={s.key} onClick={function () { togglePS(p.id, s.key); }} style={{ padding: '3px 7px', borderRadius: 6, border: '2px solid ' + s.color + (on ? '' : '22'), background: on ? s.color + '22' : '#232532', color: s.color, fontWeight: 700, fontSize: 9, cursor: 'pointer' }}>{s.emoji + ' ' + s.key}</button>;
                    })}
                  </div>
                </div>
                <div>
                  ...
                  {!showSk && <div style={{ fontSize: 9, color: '#9397ab' }}>{(p.pSk || []).length ? (p.pSk || []).map(skLabel).join(', ') : 'не выбраны'}</div>}
                  {showSk && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {ALL_SK.map(function (n) {
                        const on = (p.pSk || []).indexOf(n) >= 0;
                        return <button key={n} onClick={function () { togglePSk(p.id, n); }} style={{ padding: '2px 6px', borderRadius: 5, border: '1px solid ' + (on ? CLR : '#34374a'), background: on ? CLR + '22' : '#232532', color: on ? CLR : '#9397ab', fontSize: 8, cursor: 'pointer' }}>{skLabel(n)}</button>;
                      })}
                    </div>
                  )}
                </div>
      ...
      <div style={{ fontSize: 9, color: '#9397ab', textAlign: 'center', padding: '4px 8px', fontStyle: 'italic' }}>
        Все типы способностей работают: «🎲 Кастомный бросок», «Убеждение +5» и «Проверка d10≥6» — кнопки в листе персонажа; «+5 к атаке» (1/день) и «Переключатель» — карточки во вкладке Бой. Описания можно править даже у встроенных классов. «— Нет —» удалить нельзя.
      </div>
    </div>
  );
}
