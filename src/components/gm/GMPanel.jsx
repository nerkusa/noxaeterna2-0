...
<div style={{display:"flex",gap:2,marginTop:3,flexWrap:"wrap"}}>
  <button onClick={function(){var nl=c.level+1;var rw=levelUpReward(nl);/* Фиксируем то, что уже потрачено/осталось ДО нового уровня — иначе
  очки с разных уровней накапливались бы в одном "полу-открытом" пуле
  навсегда и игрок мог бы бесконечно перекидывать даже самые старые
  из них (абуз прокачки). */pr.saveChar(c._fbId,Object.assign({},c,{level:nl,lockedStats:Object.assign({},c.stats),lockedSkills:Object.assign({},c.skills),statPts:(c.statPts||0)+rw.stat,skillPts:(c.skillPts||0)+rw.skill,levelUpPending:{level:nl,stat:rw.stat,skill:rw.skill}}))}} title={"Повысить уровень (награда за ур."+(c.level+1)+": "+levelUpReward(c.level+1).stat+" очк. хар. + "+levelUpReward(c.level+1).skill+" очк. нав.)"} style={{padding:"3px 6px",borderRadius:4,border:"1px solid "+(xpp.ready?"#10b981":"#10b98128"),background:xpp.ready?"#10b981":"#0e2018",fontSize:8,fontWeight:700,color:xpp.ready?"#fff":"#34d399",cursor:"pointer"}}>⬆️ Ур.+1</button>
