...
var showBest=pr.showBest||false;var sShowBest=pr.setShowBest||function(){};var _rv=useState(false);var rv=_rv[0];var sRV=_rv[1];var _sh=useState(false);var shp=_sh[0];var sShp=_sh[1];var _pe=useState(false);var pe=_pe[0];var sPE=_pe[1];var _ct=useState(false);var ct=_ct[0];var sCt=_ct[1];var _tr=useState(false);var tr=_tr[0];var sTr=_tr[1];
...
if(ct)return(<div style={{flex:1,display:"flex",flexDirection:"column",minHeight:0}}><div style={{padding:"10px 14px",borderBottom:"1px solid var(--color-divider)",display:"flex",alignItems:"center",gap:8}}><button onClick={function(){sCt(false)}} className="n-btn" style={{padding:"4px 8px",fontSize:14}}>←</button><span style={{fontWeight:600,fontSize:14,flex:1}}>💬 Чат</span>{pr.clearChat&&<button onClick={function(){if(window.confirm("Очистить весь чат?"))pr.clearChat()}} className="n-btn" style={{padding:"4px 8px",fontSize:11,color:"#ef4444"}}>Очистить</button>}</div><div style={{flex:1,minHeight:0,padding:12}}><ChatTab chat={pr.chat} logs={pr.logs} sendChat={pr.sendChat} addLog={pr.addLog} characters={pr.characters} who="Мастер"/></div></div>);
