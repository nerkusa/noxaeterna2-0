...
import { r1, sm, uid, rollHit } from '../../utils/dice';
...
function rollDefense(){
  ...
  if(pr.addLog)pr.addLog({who:c.name||"???",type:"dodge",label:"🛡️ Бросок защиты"+(R.crit?" 🌟":R.fumble?" 💀":""),detail:"🎲"+d+" + DEX("+dv+") + Уклонение("+dg+") = "+t,total:t});
  if(pr.onRoll)pr.onRoll({label:"🛡️ Защита",d10:d,crit:R.crit,fumble:R.fumble,parts:[{label:"DEX",value:dv},{label:"Уклонение",value:dg}],total:t});
}

return <div style={{background:"#232532",border:"2px solid #64748b18",borderRadius:9,padding:"7px 8px"}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
<label style={S.lb}>🛡️ Броня, щит и защита</label>
<div style={{display:"flex",gap:4}}>
<button onClick={rollDefense} title="Бросок защиты: DEX + Уклонение" style={{fontSize:9,background:"#0e1a2b",border:"1px solid #3b82f640",borderRadius:5,padding:"2px 8px",cursor:"pointer",color:"#60a5fa",fontWeight:700}}>🎲 Защита</button>
<button onClick={function(){sSAA(!saa)}} style={{fontSize:9,background:"#0e2018",border:"1px solid #10b98140",borderRadius:5,padding:"2px 8px",cursor:"pointer",color:"#34d399",fontWeight:700}}>{saa?"✕":"➕"}</button>
</div>
</div>
<ShopPicker color="#10b981" label="➕ Добавить броню" items={(pr.shop||[]).filter(function(i){return i.cat==="armor"})} subOf={function(it){return it.slot||"body"}} suborder={["head","body"]} sublabels={{head:"🧠 Голова",body:"🫀 Тело"}} sub={function(it){var a=ARMOR_T.find(function(x){return x.id===it.type});return (a?a.name:it.type)+" · "+it.hp+" HP"}} onPick={function(it){var at2=ARMOR_T.find(function(a){return a.id===it.type})||ARMOR_T[0];if(fs.BODY<at2.bodyReq){alert("BODY("+fs.BODY+")<"+at2.bodyReq);return}var slot=it.slot||"body";var newA={id:uid(),name:it.name,type:it.type,slot:slot,hp:it.hp,maxHp:it.hp};var upd={armors:(c.armors||[]).concat([newA])};var curEqId=slot==="head"?c.equippedHead:c.equippedBody;if(!curEqId)upd[slot==="head"?"equippedHead":"equippedBody"]=newA.id;sv(Object.assign({},c,upd))}}/>
{saa&&<div style={{background:"#232532",borderRadius:7,padding:6,marginBottom:5,display:"flex",flexDirection:"column",gap:3}}>
<input style={S.inp} value={an} onChange={function(e){sAN(e.target.value)}} placeholder="Название брони"/>
<div style={{display:"flex",gap:3}}>
<select value={asl} onChange={function(e){sAsl(e.target.value)}} style={Object.assign({},S.inp,{flex:1,fontSize:9,padding:3,cursor:"pointer"})}><option value="head">Голова</option><option value="body">Тело</option></select>
<select value={at} onChange={function(e){sAT(e.target.value)}} style={Object.assign({},S.inp,{flex:1,fontSize:9,padding:3,cursor:"pointer"})}>{ARMOR_T.filter(function(a){return a.id!=="none"}).map(function(a){return <option key={a.id} value={a.id}>{a.name+" (Body≥"+a.bodyReq+")"}</option>})}</select>
<input style={Object.assign({},S.inp,{width:45,fontSize:9,padding:3})} type="number" value={ah} onChange={function(e){sAH(parseInt(e.target.value)||1)}} placeholder="HP"/>
</div>
<button onClick={function(){if(!an.trim())return;var at2=ARMOR_T.find(function(a){return a.id===at})||ARMOR_T[0];if(fs.BODY<at2.bodyReq){alert("BODY("+fs.BODY+")<"+at2.bodyReq);return}var newA={id:uid(),name:an.trim(),type:at,slot:asl,hp:ah,maxHp:ah};var upd={armors:(c.armors||[]).concat([newA])};var curEqId=asl==="head"?c.equippedHead:c.equippedBody;if(!curEqId)upd[asl==="head"?"equippedHead":"equippedBody"]=newA.id;sv(Object.assign({},c,upd));sAN("");sSAA(false)}} style={{padding:5,borderRadius:5,border:"none",background:"#10b981",color:"#fff",fontWeight:700,fontSize:10,cursor:"pointer"}}>Добавить</button>
</div>}
...
{shields.length===0&&!ssAdd&&<div style={{fontSize:8,color:"#9397ab",fontStyle:"italic"}}>Нет щитов</div>}
...
return <div key={(sh.id!=null?sh.id:"sh")+"_"+shIdx} style={{background:isEqSh?"#0e2236":"#1b1d29",border:"1px solid "+(isEqSh?"#38bdf830":"#34374a"),borderRadius:5,padding:"3px 5px",marginBottom:3,display:"flex",flexDirection:"column",gap:2}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
<span style={{fontSize:9,fontWeight:isEqSh?700:400}}>{sh.name} <span style={{fontSize:7,color:"#9397ab"}}>{shTyp.name+" "+shTyp.absorb*100+"%"}</span></span>
...
<div style={{background:"#232532",borderRadius:2,height:4,overflow:"hidden"}}><div style={{height:"100%",width:shPct+"%",background:sh.hp<=0?"#ef4444":"#38bdf8",borderRadius:2}}/></div>
{sh.hp<=0&&<div style={{fontSize:7,color:"#ef4444",fontWeight:700}}>💔 Сломан!</div>}
</div>;})}
</div>
{["head","body"].map(function(slot){var eq=slot==="head"?eqH:eqB;var atD=eq?ARMOR_T.find(function(a){return a.id===eq.type})||ARMOR_T[0]:null;var pct=eq&&eq.maxHp>0?(eq.hp/eq.maxHp)*100:0;return <div key={slot} style={{marginTop:4,padding:"4px 6px",background:eq?"#0e2018":"#1b1d29",border:"1px solid "+(eq?"#10b98120":"#34374a"),borderRadius:6}}><div style={{fontSize:8,fontWeight:700,color:"#9397ab"}}>{slot==="head"?"🧠 Голова":"🫀 Тело"}</div>{eq?<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:10,fontWeight:700}}>{eq.name} <span style={{fontSize:8,color:"#9397ab"}}>{atD?atD.name:""}</span></span><div style={{display:"flex",alignItems:"center",gap:2}}><button onClick={function(){sv(Object.assign({},c,{armors:(c.armors||[]).map(function(a){return a.id===eq.id?Object.assign({},a,{hp:Math.max(0,a.hp-1)}):a})}))}} style={Object.assign({},S.sm,{width:16,height:16,fontSize:8})}>−</button><span style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:11,color:eq.hp<=0?"#ef4444":"#10b981"}}>{eq.hp+"/"+eq.maxHp}</span><button onClick={function(){sv(Object.assign({},c,{armors:(c.armors||[]).map(function(a){return a.id===eq.id?Object.assign({},a,{hp:Math.min(a.maxHp,a.hp+1)}):a})}))}} style={Object.assign({},S.sm,{width:16,height:16,fontSize:8})}>+</button><button onClick={function(){var u={};u[slot==="head"?"equippedHead":"equippedBody"]=null;sv(Object.assign({},c,u))}} style={{fontSize:8,background:"#311717",border:"1px solid #ef444420",borderRadius:4,padding:"1px 5px",cursor:"pointer",color:"#ef4444",fontWeight:700}}>Снять</button></div></div><div style={{background:"#232532",borderRadius:3,height:6,overflow:"hidden",marginTop:2}}><div style={{height:"100%",width:pct+"%",background:eq.hp<=0?"#ef4444":"#10b981",borderRadius:3}}/></div>{atD&&atD.desc&&<div style={{fontSize:7,color:"#9397ab",marginTop:1}}>{atD.desc}</div>}</div>:<div style={{fontSize:9,color:"#9397ab",fontStyle:"italic"}}>Пусто</div>}</div>})}
{(c.armors||[]).length>0&&<div style={{marginTop:5}}><div style={{fontSize:8,fontWeight:700,color:"#9397ab",marginBottom:2}}>Инвентарь брони:</div>{(c.armors||[]).map(function(a,aIdx){var slot=a.slot||"body";var isEq=a.id===(slot==="head"?c.equippedHead:c.equippedBody);var atD2=ARMOR_T.find(function(x){return x.id===a.type})||ARMOR_T[0];return <div key={(a.id!=null?a.id:"a")+"_"+aIdx} style={{display:"flex",alignItems:"center",gap:3,padding:"2px 4px",borderRadius:4,background:isEq?"#12233a":"#1b1d29",marginBottom:2,fontSize:9}}><span style={{flex:1,fontWeight:isEq?700:400}}>{a.name} <span style={{fontSize:7,color:"#9397ab"}}>{SLOT_LABEL[slot]+" · "+atD2.name+" "+a.hp+"/"+a.maxHp}</span>{isEq&&<span style={{fontSize:7,color:"#3b82f6"}}> ЭКИП</span>}</span>{!isEq&&<button onClick={function(){if(fs.BODY<atD2.bodyReq){alert("BODY<"+atD2.bodyReq);return}var u={};u[slot==="head"?"equippedHead":"equippedBody"]=a.id;sv(Object.assign({},c,u))}} style={{fontSize:7,padding:"1px 6px",borderRadius:3,border:"1px solid #3b82f620",background:"#0e1a2b",cursor:"pointer",color:"#60a5fa"}}>{"Надеть ("+SLOT_LABEL[slot]+")"}</button>}<button onClick={function(){if(!window.confirm("Удалить "+a.name+"?"))return;var na=(c.armors||[]).filter(function(x,j){return j!==aIdx});var has=function(id){return na.some(function(x){return x.id===id})};sv(Object.assign({},c,{armors:na,equippedHead:has(c.equippedHead)?c.equippedHead:null,equippedBody:has(c.equippedBody)?c.equippedBody:null}))}} title="Удалить" style={{fontSize:10,background:"none",border:"none",color:"#ef4444",cursor:"pointer"}}>🗑</button></div>})}</div>}
{(function(){
  ...
  return <div style={{marginTop:6,padding:"5px 7px",background:"#231b08",border:"1px solid #f59e0b30",borderRadius:7}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:8,fontWeight:700,color:"#f0b352"}}>🔧 Ремонт снаряжения (1/день)</span><span style={{fontSize:7,color:"#9397ab"}}>{used?"✓ использовано":canRepair?pwr:"нужны инструменты"}</span></div>
    {!used&&canRepair&&ownDmg.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:2,marginTop:3}}>{ownDmg.map(function(it){return <button key={it.kind+it.id} onClick={function(){repOwn(it.kind,it.id,it.nm)}} style={{padding:"2px 5px",borderRadius:4,border:"1px solid #f59e0b30",background:"#1b1d29",fontSize:7,fontWeight:700,color:"#f0b352",cursor:"pointer"}}>{"🔧 "+it.nm+" ("+it.hp+"/"+it.mhp+")"}</button>})}</div>}
    {!used&&canRepair&&ownDmg.length===0&&!allies.some(function(a){return dmgOf(a).length})&&<div style={{fontSize:7,color:"#9397ab",marginTop:2,fontStyle:"italic"}}>Вся броня цела</div>}
    {!used&&!canRepair&&<div style={{fontSize:7,color:"#9397ab",marginTop:2,fontStyle:"italic"}}>Возьми набор инструментов в инвентаре (или будь Ремесленником)</div>}
    {isArt&&!used&&allies.map(function(al){var ad=dmgOf(al);if(!ad.length)return null;return <div key={al._fbId} style={{marginTop:3}}><div style={{fontSize:7,color:"#9397ab",fontWeight:700}}>{"🤝 "+al.name}</div><div style={{display:"flex",flexWrap:"wrap",gap:2,marginTop:1}}>{ad.map(function(it){return <button key={it.kind+it.id} onClick={function(){repAlly(al,it.kind,it.id,it.nm)}} style={{padding:"2px 5px",borderRadius:4,border:"1px solid #10b98130",background:"#0e2018",fontSize:7,fontWeight:700,color:"#34d399",cursor:"pointer"}}>{"🔧 "+it.nm+" ("+it.hp+"/"+it.mhp+")"}</button>})}</div></div>})}
    {used&&<div style={{fontSize:7,color:"#9397ab",marginTop:2,fontStyle:"italic"}}>Отдых (💤) сбросит починку</div>}
  </div>;
})()}
</div>}
