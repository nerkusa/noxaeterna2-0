...
<ShopPicker color="#38bdf8" label="Добавить щит" items={(pr.shop||[]).filter(function(i){return i.cat==="shield"})} subOf={function(it){return it.type}} suborder={["light","medium","tower"]} sublabels={{light:"🛡 Щит лёгкий",medium:"🛡 Щит средний",tower:"🛡 Щит тяжёлый"}} sub={function(it){var t=SHIELD_T.find(function(x){return x.id===it.type});return (t?t.name+" "+(t.absorb*100)+"%":it.type)+" · "+it.hp+" HP"+(it.desc?" · "+it.desc:"")}} onPick={function(it){var t=SHIELD_T.find(function(x){return x.id===it.type})||SHIELD_T[0];if(fs.BODY<t.bodyReq){alert("BODY("+fs.BODY+")<"+t.bodyReq);return}var pay=tryPay(c,it.price);if(pay===null)return;sv(Object.assign({},c,pay,{shields:(c.shields||[]).concat([{id:uid(),name:it.name,type:it.type,absorb:t.absorb,hp:it.hp,maxHp:it.hp,desc:it.desc||""}])}))}}/>
