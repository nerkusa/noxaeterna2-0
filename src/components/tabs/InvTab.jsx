...
function pickItem(it){
  ...
  if(ptype){
    var inv=(c.inventory||[]).slice();var ex=inv.find(function(i){return i.proj&&i.ptype===ptype});
    ...
    sv(Object.assign({},c,{inventory:inv}));sSpOpen(false);return;
  }
  if(dice){sv(Object.assign({},c,{inventory:(c.inventory||[]).concat([{id:uid(),name:it.name,qty:1,equipped:false,tool:true,dice:dice}])}));sSpOpen(false);return;}
  sv(Object.assign({},c,{inventory:(c.inventory||[]).concat([{id:uid(),name:it.name,qty:1,equipped:false}])}));sSpOpen(false);
}
...
{shopItems.map(function(it){var ptype=it.ptype||(it.cat==="ammo"?"Стрела":"");var dice=it.dice||(it.cat==="tool"?"1d4":"");
...
<div style={{background:"#2a2008",borderRadius:8,padding:"5px 8px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:10}}>💰</span><div style={{display:"flex",alignItems:"center",gap:2}}>{[-10,-1].map(function(d){return <button key={d} onClick={function(){sv(Object.assign({},c,{gold:Math.max(0,(c.gold||0)+d)}))}} style={S.sm}>{d}</button>})}<span style={{fontFamily:"'Inter',sans-serif",fontSize:14,fontWeight:700,color:"#d97706",minWidth:24,textAlign:"center"}}>{c.gold||0}</span>{[1,10].map(function(d){return <button key={d} onClick={function(){sv(Object.assign({},c,{gold:(c.gold||0)+d}))}} style={S.sm}>{"+"+d}</button>})}</div></div>
