...
var TOKEN_COLORS = ["#ef4444", "#f97316", "#f59e0b", "#eab308", "#84cc16", "#10b981", "#06b6d4", "#3b82f6", "#6366f1", "#8b5cf6", "#ec4899", "#e9e9ed", "#ffffff", "#a16207", "#0f766e", "#7c3aed", "#be185d", "#b45309", "#b2b6ca", "#dc2626"];
...
var mapSwitch=(<div style={{display:"flex",gap:4,marginBottom:2}}>
  <button onClick={function(){sView("local");sZoom(1)}} style={{flex:1,padding:"6px 4px",borderRadius:7,border:"2px solid #10b981"+(isWorld?"20":""),background:isWorld?"#1b1d29":"#0e2018",color:"#34d399",fontWeight:700,fontSize:10,cursor:"pointer"}}>🏠 Локальная</button>
  <button onClick={function(){sView("world");sZoom(1)}} style={{flex:1,padding:"6px 4px",borderRadius:7,border:"2px solid #3b82f6"+(isWorld?"":"20"),background:isWorld?"#0e1a2b":"#1b1d29",color:"#60a5fa",fontWeight:700,fontSize:10,cursor:"pointer"}}>🌍 Масштабная (1:1)</button>
</div>);
var mapTitle=md.title||"Карта Мидфаля";
var titleBar=isGM
  ?<input value={md.title||""} onChange={function(e){if(sMD)sMD(Object.assign({},md,{title:e.target.value}))}} placeholder="Карта Мидфаля" style={{width:"100%",boxSizing:"border-box",textAlign:"center",fontFamily:"'Inter',sans-serif",fontWeight:900,fontSize:15,background:"#1b1d29",color:"#e9e9ed",border:"2px solid #34374a",borderRadius:8,padding:"4px 8px",outline:"none",marginBottom:4}}/>
  :<div style={{textAlign:"center",fontFamily:"'Inter',sans-serif",fontWeight:900,fontSize:15,color:"#e9e9ed",marginBottom:4}}>{mapTitle}</div>;
...
if(!img)return <div style={{display:"flex",flexDirection:"column",gap:6}}>{titleBar}{mapSwitch}<div style={{textAlign:"center",padding:"24px 10px"}}><div style={{fontSize:40}}>🗺️</div><div style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:14}}>{isWorld?"Масштабная карта (1:1)":"Локальная карта"}</div>{isGM?<label style={{display:"inline-block",marginTop:10,padding:"10px 20px",borderRadius:8,border:"2px dashed #10b981",background:"#0e2018",fontWeight:700,fontSize:12,color:"#34d399",cursor:"pointer"}}>📁 Загрузить карту<input type="file" accept="image/*" style={{display:"none"}} onChange={function(e){var f=e.target.files&&e.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(ev){setImg(ev.target.result)};r.readAsDataURL(f)}}/></label>:<div style={{fontSize:10,color:"#9397ab",marginTop:8}}>Мастер ещё не загрузил карту</div>}</div></div>;
return <div style={{display:"flex",flexDirection:"column",gap:6}}>
{titleBar}
{mapSwitch}
<div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
  <div style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:13}}>{isWorld?"🌍 Масштабная":"🏠 Локальная"}</div>
  <div style={{display:"flex",alignItems:"center",gap:4}}>
    <button onClick={function(){sZoom(function(z){return Math.max(0.25,+(z-0.25).toFixed(2))})}} style={{width:24,height:24,borderRadius:5,border:"1px solid #34374a",background:"#1b1d29",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
    <span style={{fontSize:9,fontWeight:700,minWidth:32,textAlign:"center"}}>{Math.round(zoom*100)+"%"}</span>
    <button onClick={function(){sZoom(function(z){return Math.min(8,+(z+0.25).toFixed(2))})}} style={{width:24,height:24,borderRadius:5,border:"1px solid #34374a",background:"#1b1d29",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
    <button onClick={function(){sZoom(1)}} style={{padding:"2px 6px",borderRadius:5,border:"1px solid #34374a",background:"#1b1d29",fontSize:8,cursor:"pointer",color:"#9397ab"}}>1:1</button>
  </div>
</div>
...
<div ref={contRef} style={{overflow:"auto",borderRadius:10,border:"2px solid #34374a",background:"#1a1a2e",maxHeight:500,touchAction:"none",userSelect:"none",cursor:pan?"grabbing":"grab"}} onMouseDown={startPan} onTouchStart={startPan} onMouseMove={onMv} onMouseUp={endDr} onMouseLeave={endDr} onTouchMove={onMv} onTouchEnd={endDr}>
...
<div style={{display:"flex",flexDirection:"column",gap:3}}>{allT.map(function(t){var sz=t.size||6;var canEdit=isGM||t.charId===cId;return <div key={t.id} style={{background:"#1b1d29",border:"1px solid #34374a",borderRadius:6,padding:"3px 6px",fontSize:8}}>
<div style={{display:"flex",alignItems:"center",gap:4}}>
<button onClick={function(){if(canEdit)sEditTk(editTk===t.id?null:t.id)}} title={canEdit?"Сменить цвет":""} style={{width:12,height:12,borderRadius:"50%",background:t.color||"#3b82f6",border:"1px solid #34374a",flexShrink:0,cursor:canEdit?"pointer":"default",padding:0}}/>
...
{canEdit&&<button onClick={function(){var nm=window.prompt("Имя жетона:",t.name||"");if(nm!==null)updTk(t.id,{name:nm.trim()||t.name})}} title="Переименовать" style={{background:"none",border:"none",cursor:"pointer",fontSize:9,color:"#9397ab",lineHeight:1}}>✎</button>}
{canEdit&&<span style={{display:"flex",gap:1,alignItems:"center"}}><button onClick={function(){updTk(t.id,{size:Math.max(2,sz-1)})}} style={{background:"none",border:"none",cursor:"pointer",fontSize:11,color:"#9397ab",lineHeight:1}}>−</button><span style={{fontSize:7,minWidth:20,textAlign:"center"}}>{sz}px</span><button onClick={function(){updTk(t.id,{size:Math.min(60,sz+1)})}} style={{background:"none",border:"none",cursor:"pointer",fontSize:11,color:"#9397ab",lineHeight:1}}>+</button></span>}
...
{canEdit&&editTk===t.id&&<div style={{display:"flex",gap:2,flexWrap:"wrap",marginTop:4,paddingTop:4,borderTop:"1px solid #34374a"}}>{TOKEN_COLORS.map(function(cl){return <button key={cl} onClick={function(){updTk(t.id,{color:cl});sEditTk(null)}} style={{width:16,height:16,borderRadius:"50%",background:cl,border:(t.color===cl)?"2px solid #b2b6ca":"1px solid #3f424d",cursor:"pointer",flexShrink:0,padding:0}}/>})}</div>}
</div>})}</div>
{!isGM&&cId&&!allT.find(function(t){return t.charId===cId})&&<div style={{display:"flex",flexDirection:"column",gap:4}}>
<div style={{display:"flex",gap:2,flexWrap:"wrap",justifyContent:"center"}}>{TOKEN_COLORS.map(function(cl){return <button key={cl} onClick={function(){sPColor(cl)}} style={{width:18,height:18,borderRadius:"50%",background:cl,border:pColor===cl?"3px solid #b2b6ca":"1.5px solid #3f424d",cursor:"pointer",flexShrink:0,padding:0}}/>})}</div>
...
<div style={{display:"flex",gap:2,flexWrap:"wrap"}}>{TOKEN_COLORS.map(function(cl){return <button key={cl} onClick={function(){sNC(cl)}} style={{width:18,height:18,borderRadius:"50%",background:cl,border:nc===cl?"3px solid #b2b6ca":"1.5px solid #3f424d",boxShadow:nc===cl?"0 0 0 2px "+cl:"none",cursor:"pointer",flexShrink:0}}/>})}</div>
<div style={{display:"flex",gap:3,alignItems:"center"}}><label style={{fontSize:8,fontWeight:700,color:"#9397ab",whiteSpace:"nowrap"}}>Размер (px):</label><input style={Object.assign({},S.inp,{width:50,fontSize:9,padding:2,textAlign:"center"})} type="number" min={2} max={60} value={ns} onChange={function(e){sNS(parseInt(e.target.value)||4)}}/><span style={{fontSize:8,color:"#9397ab"}}>мин 2, макс 60</span></div>
