...
const backBtn = { alignSelf: 'flex-start', padding: '5px 12px', borderRadius: 6, border: '2px solid #34374a', background: '#1b1d29', color: '#e9e9ed', fontWeight: 700, fontSize: 11, cursor: 'pointer' };
  ...
  return (
    <button onClick={pr.onClick} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, border: '2px solid ' + color + '22', background: color + '0e', cursor: 'pointer', textAlign: 'left' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 12, color: '#e9e9ed' }}>{meta.title}</div>
        {pr.subtitle && <div style={{ fontSize: 9, color: '#9397ab', marginTop: 1 }}>{pr.subtitle}</div>}
      </div>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: "'Inter',sans-serif", fontWeight: 900, fontSize: 16, color: sec.color, marginTop: 4 }}>{sec.title}</div>
        </div>
        {hasContent
          ? <LoreContent section={sec} />
          : <div style={{ textAlign: 'center', padding: 20, color: '#9397ab' }}><div style={{ fontSize: 24 }}>📜</div><div style={{ fontSize: 12, fontWeight: 700 }}>Пусто</div></div>}
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ textAlign: 'center', padding: '8px 0' }}>
        <div style={{ fontFamily: "'Inter',sans-serif", fontWeight: 900, fontSize: 17 }}>📚 Лорбук</div>
      </div>
      ...
      {sections.length === 0 && <div style={{ textAlign: 'center', padding: 16, color: '#9397ab', fontSize: 11 }}>Разделов пока нет</div>}
    </div>
  );
}
