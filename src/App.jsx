...
var _gml=useState([]);var gmLog=_gml[0];var sGmLog=_gml[1];
...
/* История собственных действий ГМ (выдача XP, левелап, правка HP и т.п.) —
   отдельно от общего чата/логов бросков, чтобы ГМ мог отследить, что сам
   поменял игрокам. */
function addGmLog(e){if(!room)return;set(ref(db,"rooms/"+room+"/gmLog/"+Date.now()),Object.assign({},e,{ts:Date.now()}))}
function clearGmLog(){if(!room)return;set(ref(db,"rooms/"+room+"/gmLog"),null)}
